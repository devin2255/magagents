...
## 🔄 任务如何流转 —— 真·美式分权制衡（12 个 Agent）

`run_task_agentic()` 驱动任务走一遍美国政府，建模**真正的分权制衡**：**没有任何一个
分支能单方面把事情推过去**。每一步都是真实 LLM 决策。

```
用户任务
   │
   ▼  立法 —— 两院制国会（两院都必须通过）
1) 🏛️ 众议院 (congress_house)  ─ reject → 🏛️ 国会未通过 (结束)
2) 🏛️ 参议院 (congress_senate) ─ reject → 🏛️ 国会未通过 (结束)
   │
   ▼  行政 —— 呈送总统
3) 🎩 总统 (trump_president) ─ 签署 → 第4步
   │                         └ 否决 → ⚖️ 国会反否决投票（2/3，两院都要）
   │                                    ├ 推翻成功 → 照样成法 → 第4步
   │                                    └ 维持否决 → ❌ 已否决 (结束)
   ▼  行政 —— 执行（自适应多部门协作）
4) 💼 总统指定【牵头部门】+ 可选【协作部门】
   │   简单任务 → 牵头部门独自产出
   │   复杂任务 → 各协作部门各出一块 → 牵头部门汇总成最终交付物
   │
   ▼  司法 —— 违宪审查
5) ⚖️ 最高法院 (scotus) ─ 维持 → 第6步
   │                     └ 判违宪 → ⚖️ 判违宪作废 (结束)
   ▼  监督
6) 🐕 DOGE (doge_musk) ─ 基于真实 token 审计 ─ pass → ✅ 已完成
                                             └ fire → 🔥 已解雇 (结束)
```

**让"制衡"成立的几道牙齿（没有分支至高无上）：**
- **两院制**：众议院 + 参议院都必须通过（两道立法关）。
- **否决 + 反否决**：总统能否决，但国会能**用两院各 2/3 多数推翻否决**（立法牵制行政）。
- **违宪审查**：最高法院能把已成法的行为**判违宪作废**（司法牵制两者）。
- **监督**：DOGE 基于真实 token 用量审计执行。

**执行是自适应的多部门协作**（这属于行政分支**内部**的配合，与分支**之间**的制衡是两回事）：
总统指定一个**牵头部门**，复杂任务再拉上最多 3 个**协作部门**——各出自己的专长那一块，
由牵头部门汇总成最终交付物；简单任务只用牵头部门，保持低成本。

**谁来干活？总统按任务类型指派内阁部门：**

| 部门 | 擅长 |
|---|---|
| `commerce` 商务部 | 写作、文案、营销、数据/市场分析 *(默认)* |
| `treasury` 财政部 | 预算、成本/ROI、财务测算 |
| `defense` 国防部 | 安全、风险、攻防、加固 |
| `state_dept` 国务院 | 对外沟通、合作、API/集成 |
| `energy` 能源部 | 算力调度、性能、资源/容量 |
| `justice` 司法部 | 合规、法律/政策审查、制度 |

每个分支都是**安全守门人而非官僚**——默认放行，只拦截**有害、违法、无意义**的任务
（"缺细节"绝不构成驳回理由）：正当的活照样顺畅通过，而制衡机制是真实存在的。

**结局：** `已完成 DONE` · `国会未通过 CONGRESS_FAILED` · `已否决 VETOED` ·
`判违宪作废 STRUCK_DOWN` · `已解雇 FIRED` · `超预算 OVER_BUDGET`（超预算时 DOGE
会在下一步前叫停）。

每一步都会把**真实 token 用量**记入 DOGE，且整条链路**回复语言跟随你的输入**。
