...
任务由 `run_task_agentic()`（或对话 CLI / 看板）驱动，走一遍美国政府流程。每一步
都是对应角色的**真实 LLM 决策**；由总统决定**派哪个内阁部门**干活。

```
用户任务
   │
   ▼
1) 🏛️ 国会 (congress_senate) ── 投票 ─┐  pass → 第3步
   │                                  └─ reject → 第2步
2) ⚖️ 最高法院 (scotus) ── 裁决 ──────┐  proceed → 第3步
   │                                  └─ block → ⛔ 已驳回 (结束)
3) 🎩 总统 (trump_president) ── 决策 ──┐ approve + 指派部门 → 第4步
   │                                  └─ veto → ❌ 已否决 (结束)
4) 💼 内阁某部门 (被指派者) ── 产出真正的交付物
   │
5) 🐕 DOGE (doge_musk) ── 基于真实 token 审计 ─┐ pass → ✅ 已完成
                                              └─ fire → 🔥 已解雇 (结束)
```
...
**三权是"安全守门人"而非官僚**——国会/最高法院/总统**默认放行**，只拦截
**有害、违法、无意义**的任务（"缺细节"绝不构成驳回理由）。两个立法机构
（`congress_senate` 参议院、`congress_house` 众议院）共同组成国会；agentic 流程
目前以**参议院**作为国会环节的检查点。

**结局：** `已完成 DONE` · `已否决 VETOED` · `已驳回 BLOCKED` · `已解雇 FIRED` ·
`超预算 OVER_BUDGET`（任务超出成本/token 预算时 DOGE 会在下一步前叫停）。

每一步都会把**真实 token 用量**记入 DOGE（审计与解雇用真数据），且整条链路
**回复语言跟随你的输入**。
